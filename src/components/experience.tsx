...
import { EXPERIENCES } from "@/constants"; // Adjust the import path to match the location of your index.ts file
import { formatDate } from "@/utils";
import { motion } from "framer-motion";
import { useState } from "react";
import { PerspectiveText, SectionHeading, SlideIn, Transition } from "./ui";

const Experience = () => {
  // Sort EXPERIENCES based on the sequence number
  const sortedExperiences = EXPERIENCES.sort((a, b) => a.sequence - b.sequence);
  ...
  return (
    <div className="relative pb-20">
      ...
      <SectionHeading className="pl-4 md:px-12 py-20">
        <SlideIn className="text-white/40">Mon</SlideIn>
        <br />
      ...
      <div>
        {sortedExperiences.map((exp, index) => (
          <Transition
            key={index} // Changed key since _id doesn't exist in the new data structure
          ...
          >
            <div className="flex items-center justify-between md:gap-8">
              ...
              <div className="md:text-5xl text-xl md:font-semibold flex-1">
                <PerspectiveText hover={hover === index}>{exp.jobTitle}</PerspectiveText>
              </div>
