...
const Filters = () => {
  const { projects, setAppliedFilter, appliedFilter, sort, setSort } = useProjects();
  const techStack = projects.flatMap((filter) => filter.techStack.map((val) => val.trim()));
...
const Sort = () => {
  ...
  return (
    <Button className="border border-white/20 px-4 py-2 rounded-full" onClick={() => setSort(true)}>
