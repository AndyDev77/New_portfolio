...
import { PROJECTS } from "@/constants"; // Ensure this import points to where your PROJECTS is defined
import { useCursorVariants } from "@/utils/context";
import { Project } from "@/utils/interface"; // Ensure this points to where your Project interface is defined
    ...
    <ProjectsProvider data={PROJECTS}>
      <section className="md:p-8 p-4 relative" id="projects">
        <SectionHeading className="md:pl-16">
          <SlideIn className="text-white/40">Mes</SlideIn>
          <br />
          <SlideIn>Projets</SlideIn>
        </SectionHeading>
        <Filters />
        <ProjectContainer />
      </section>
...
const Card = ({ title, image, description, techStack, liveurl, githuburl }: Project) => {
