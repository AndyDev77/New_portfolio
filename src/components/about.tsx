...
import { ABOUT, TIMELINE } from "@/constants"; // Adjust the import path to match the location of your index.ts file
...
const About = () => {
  ...
  const about = ABOUT[0];
  const timeline = TIMELINE; // Assuming TIMELINE is directly usable without filtering for education only
...
const TimelineCard = ({ timeline, activeIndex, setActiveIndex, index }: TimelineCardProps) => (
  <div className="border-b border-primary/20 py-4">
    ...
    >
      <p className="text-foreground/60 py-4 max-md:text-sm">{timeline.summary}</p>
